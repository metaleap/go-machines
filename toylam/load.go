...
// for compilers or other syntax users. defaults to all-`false`s for our interpreter in here
...
func (me *Loc) LocInfo() *Loc { return me }
func (me *Loc) LocStr() string {
	if me == nil {
		return ""
	}
	return "in '" + me.ModuleName + "." + me.TopDefName + "', line " + strconv.Itoa(me.LineNr) + ": "
}
...
func (me *Prog) ParseModules(modules map[string][]byte, opts ParseOpts) {
	...
	me.exprId, me.exprListNil, me.exprListConsCtorBody = me.TopDefs[StdRequiredDefs_id].(*ExprFunc), me.TopDefs[StdRequiredDefs_listNil].(*ExprFunc), me.TopDefs[StdRequiredDefs_listCons].(*ExprFunc).Body.(*ExprFunc).Body.(*ExprFunc).Body
	if !opts.KeepOpRefs {
...
func (me *ctxParse) parseTopDef(lines []string, idxStart int, idxEnd int) (topDefName string, topDefBody Expr, firstLn string) {
	...
	for i, ln := range lines[idxStart:idxEnd] {
		if ln = strings.TrimSpace(ln); ln != "" {
			lnorig, loc := ln, &Loc{me.curModule.name, topDefName, 1 + i + idxStart}
			...
			if idx := strings.Index(ln, ":="); idx < 0 {
				...
				firstLn, topDefName, loc.TopDefName, topdefargs = lnorig, defsig[0], defsig[0], defsig[1:]
...
func (me *ctxParse) parseExpr(src string, locHintLn string, locInfo *Loc) (expr Expr) {
...
func (me *ctxParse) parseExprToks(toks []string, locHintLn string, locInfo *Loc) (expr Expr) {
	if len(toks) == 0 {
...
func (me *ctxParse) populateNames(expr Expr, binders map[string]int, curModule map[string]Expr, locHintTopDefName string) Expr {
	...
	case *ExprName:
		...
		if posdot := strings.LastIndexByte(it.NameVal, '.'); posdot > 0 && nil == me.srcs[it.NameVal[:posdot]] && (nil == me.srcs[stdpref+it.NameVal[:posdot]] || 0 != binders[it.NameVal[:posdot]]) {
			...
			var ret Expr = &ExprName{it.Loc, dotpath[0], 0}
			for i := 1; i < len(dotpath); i++ {
				ret = &ExprCall{it.Loc, &ExprName{it.Loc, dotpath[i], int(instrs[dotpath[i]])}, ret}
			}
	...
	case *ExprFunc:
		if _, topdefexists := curModule[it.ArgName]; topdefexists || binders[it.ArgName] != 0 {
			panic("in '" + me.curModule.name + "." + locHintTopDefName + "', line " + strconv.Itoa(it.LineNr) + ": local name '" + it.ArgName + "' already taken: " + it.String())
		}
...
func (me *ctxParse) extractBrackets(loc *Loc, ln string, lnOrig string, needLegalName int) string {
		...
		if it.IdxOrInstr <= 0 {
			...
			if it.IdxOrInstr == 0 {
				...
				if !me.opts.KeepNameRefs {
					if topdefbody == nil {
						panic("in '" + topDefQName + "', line " + strconv.Itoa(it.LineNr) + ": name '" + it.NameVal + "' unresolvable in:\n" + topDefBody.String())
				...
				panic("in '" + topDefQName + "', line " + strconv.Itoa(it.LineNr) + ": local name '" + it.NameVal + "' already taken (no shadowing allowed)")
			}
		}
	}
	return expr
	...
	if name, _ := me.Callee.(*ExprName); name != nil && name.IdxOrInstr > 0 {
		if lhs, _ := me.CallArg.(*ExprLitNum); lhs != nil {
			if instr := Instr(name.IdxOrInstr); instr < InstrEQ {
				ok, rhs.Loc, rhs.NumVal = true, parent.Loc, int(instr.callCalc(parent.Loc, valNum(lhs.NumVal), valNum(rhs.NumVal)))
			}
		}
	}
	return
